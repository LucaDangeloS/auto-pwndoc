...
- `backend/src/models/vulnerability-matching-run.js`: persisted background proposal runs for automatic translation matching. Runs store progress plus pending/accepted/dismissed proposals so the UI can recover after page refresh or backend restart. Applying proposals creates/updates `VulnerabilityTranslationGroup` records and is idempotent for already-accepted proposals. Matching thresholds are distance cutoffs; the explicit start payload wins, then `settings.ai.public.vulnerabilityProcessing.matchThreshold`, then `embeddingMaxDistance`. `DELETE /api/vulnerabilities` clears vulnerabilities, translation groups, and matching runs together so imports start with no mapping state. `POST /api/vulnerabilities/translation-groups/cleanup` removes stale members left by deleted or changed vulnerability details and drops groups with fewer than two valid members.
- Vulnerability auto-translation has two paths. Missing language details on the same vulnerability can be generated manually with `POST /api/vulnerabilities/:id/translations/auto-translate-locale` or automatically on create/update when `settings.ai.public.vulnerabilityProcessing.autoTranslateOnSave` is enabled. Related vulnerability groups still sync best-effort per related target; successful target languages are updated and marked `pending-review`, while failed targets are marked `failed` and reported in the route response without aborting the rest of the translation group.
- Custom field `fieldType: 'checklist'` stores `field.text[locale].value` as an array of `{label, code, taxonomy: {type, category, subcategory}, status, note}`. The audit/finding/section instance copies this array forward at create time; `Audit.applyChecklistAutoMark` is invoked from `createFinding` / `updateFinding` and flips a row's `status` from `untested` to `fail` when the finding's `taxonomies[]` match the row's taxonomy or `code`. Pass/fail/na set by a human are never overwritten. Generate-from-taxonomy uses `POST /api/data/vulnerability-taxonomy/generate-checklist`.
